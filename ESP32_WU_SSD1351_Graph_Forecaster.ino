...
float pressure_grad, temperature_grad, humidity_grad;
...
void setup() { 
  // To include a different font use:
...
void loop() {
  ...
  if(CurrCondObj(&currCondString)) {
    ...
    String forecast_time = getLocalTime();
    Serial.println(icon);
    if (icon == "snow" ||
        icon == "chanceflurries" ||
        icon == "chancesnow" || 
        ...
        icon == "nt_chancesnow" || 
        icon == "nt_flurries" ||
        icon == "nt_snow")           { drawWximage(0,20,snow_img); }
    if (icon == "rain" || 
        icon == "chancerain" ||
        icon == "nt_chancerain" ||
        icon == "nt_rain")           { drawWximage(0,20,rain_img); }
    if (icon == "sleet" || 
        icon == "chancesleet" ||
        icon == "nt_chancesleet" ||
        icon == "nt_sleet")          { drawWximage(0,20,sleet_img); }
    if (icon == "sunny" ||
        icon == "clear")             { drawWximage(0,20,sunny_img); }
    if (icon == "cloudy" ||
        icon == "nt_cloudy")         { drawWximage(0,20,cloudy_img); }
    if (icon == "mostlysunny" ||
        icon == "partlycloudy" ||
        icon == "partlysunny" ||
        icon == "mostlycloudy")      { drawWximage(0,20,mostlysunny_img); }
    if (icon == "fog" ||
        icon == "nt_fog" ||
        icon == "nt_hazy" ||
        icon == "hazy")              { drawWximage(0,20,fog_img); }
    if (icon == "tstorms" ||
        icon == "chancetstorms" ||
        icon == "nt_tstorms" ||
        icon == "nt_chancetstorms")  { drawWximage(0,20,tstorms_img); }
    if (icon == "nt_mostlycloudy" ||
        icon == "nt_mostlysunny" ||
        icon == "nt_partlycloudy" ||
        icon == "nt_partlysunny")    { drawWximage(0,20,cloudy_img); }
    if (icon == "nt_clear")          { drawWximage(0,20,sunny_img); }
        
    display_text(0, 0 ,"WX: "+forecast_time.substring(0,12)+forecast_time.substring(14,22),YELLOW,1,28);
    if (getCurrWeather() == "Rain") display_text(0,10,getCurrWeather()+" ("+getPrecipTodayMet()+"mm)",YELLOW,1,16); 
    else display_text(0,10,getCurrWeather(),YELLOW,1,16);
    
    display_text(65,40,getCurrC()+char(247)+"C",ORANGE,1,6); // use getCurrF for fahrenheit
    display_text(65,70,getRelHum(),ORANGE,1,3); 
    display_text(0,65,getWindDir()+" @ "+getWindMPH()+"mph",MAGENTA,1,15);
    display_text(0,65,getPressure_mb()+" hPa ",MAGENTA,1,10);
    display_text(50,70,Pressure_trend(),MAGENTA,1,8);
    ...
    predict_weather();
    display_text(230,0,"Trend Charts",CYAN,1,12);
    draw_readings(80,20,30,20,8,pressure);    // x,y position, width, height, number of bars
    draw_readings(80,60,30,20,8,temperature); // x,y position, width, height, number of bars
    draw_readings(80,100,30,20,8,humidity);   // x,y position, width, height, number of bars
  }
  delay(30000);
}
...
String Pressure_trend() {
  ...
  if (pTrend == "+") return "(Rising)";
  else if
     (pTrend == "0") return "(Steady)";
  else if
     (pTrend == "-") return "(Falling)";
  else return "";
} 
...
void predict_weather(){
  ...
  float  trend = pressure_now - reading[pressure][22]; // trend of pressure over last 1-hour
  
  if (trend >= 3.5)                        {trend_str = "Rising rapidly";  drawWximage(0,70,sunny_img); }
  else if (trend >=  0.1  && trend <= 1.5) {trend_str = "Rising";          drawWximage(0,70,sunny_img); }
  else if (trend >   0.1  && trend <= 1.5) {trend_str = "Rising slowly";   drawWximage(0,70,sunny_img); }
  else if (trend >  -0.1  && trend < 0.1)  {trend_str = "Steady";          drawWximage(0,70,mostlysunny_img); }
  else if (trend >= -1.5  && trend <-0.1)  {trend_str = "Falling slowly";  drawWximage(0,70,cloudy_img); }
  else if (trend >= -3.5  && trend < -1.5) {trend_str = "Falling";         drawWximage(0,70,rain_img); }
  else if (trend <= -3.5)                  {trend_str = "Falling rapidly"; drawWximage(0,70,tstorms_img); }

  display_text(6,90,String(pressure_now),CYAN,1,15);
  display_text(6,100,trend_str,CYAN,1,15);
  display_text(6,110,String(trend),CYAN,1,15);
      
  if (pressure_now >= 1022.68 )    
    forecast_text = "Good clear weather";   
  if (pressure_now >= 1022.7  && trend_str  == "Falling rapidly")      
    forecast_text = "Warmer, rain within 36-hrs";   
  if (pressure_now >= 1013.2  && pressure_now <= 1022.68 && (trend_str == "Steady" || trend_str == "Rising slowly"))    
    forecast_text = "No change";    
  if (pressure_now >= 1013.2  && pressure_now <= 1022.68 && (trend_str == "Rising" || trend_str == "Rising rapidly"))   
    forecast_text = "Getting warmer";   
  if (pressure_now >= 1013.2  && pressure_now <= 1022.68 &&  trend_str == "Rising slowly")    
    forecast_text = "Becoming clearer";   
  if (pressure_now >= 1013.2  && pressure_now <= 1022.68 && (trend_str == "Falling slowly" || trend_str == "Falling rapidly") )    
    forecast_text = "Expect rain";    
  if (pressure_now >= 1013.2  && pressure_now <= 1022.68 && trend_str  == "Steady")    
    forecast_text = "Clear spells";
  if (pressure_now <= 1013.2  && (trend_str == "Falling slowly" || trend_str == "Falling"))    
    forecast_text = "Rain in 18-hrs";   
  if (pressure_now <= 1013.2  &&  trend_str == "Falling rapidly")    
    forecast_text = "Rain, high winds, clear and cool";   
  if (pressure_now <= 1013.2  && (trend_str == "Rising" || trend_str=="Rising slowly"||trend_str=="Rising rapidly"))   
    forecast_text = "Clearing within 12-hrs";   
  if (pressure_now <= 1009.14 && trend_str  == "Falling rapidly")    
    forecast_text = "Gales, heavy rain, snow in winter";    
  if (pressure_now <= 1009.14 && trend_str  == "Rising rapidly")   
    forecast_text = "Clearing and colder";    
  display_text(0,40,forecast_text,YELLOW,1,10);
}
...
void draw_readings(int x, int y, int gwidth, int gscale, int num_bars, int entity){
  ...
  if ((press_hour > last_press_hour)||(press_hour<last_press_hour)) {
    for (int i = 0; i < 23;i++){
    ...
    pressure_grad    = reading[pressure][22]/reading[pressure][23];       // 1 +/- 2% is steady, less than 1 is falling and greater than 1 climbing
    temperature_grad = reading[temperature][22]/reading[temperature][23]; // 1 +/- 2% is steady, less than 1 is falling and greater than 1 climbing
    humidity_grad    = reading[humidity][22]/reading[humidity][23];       // 1 +/- 2% is steady, less than 1 is falling and greater than 1 climbing
  }
  ...
  if (entity == pressure)    display_text(x+5,y-9,"Pres hPa",CYAN,1,8);
  if (entity == temperature) display_text(x+5,y-9,"Temp C",CYAN,1,11);
  if (entity == humidity)    display_text(x+5,y-9,"Humi %",CYAN,1,8);
  display_text(x+2+(bar_width+3)*0, y+gscale+2,"-24",CYAN,1,4); // 24hr marker at bar 0
  //display_text(x+2+(bar_width+3)*2, y+gscale+2,"-12",CYAN,1,4); // 12hr marker at bar 2
  //display_text(x+2+(bar_width+3)*5, y+gscale+2,"-2", CYAN,1,2); // 2hr  marker at bar 5
  display_text(x+2+(bar_width+3)*7, y+gscale+2,"0",  CYAN,1,1); // 0hr  marker at bar 7
  //display.drawFastHLine(x, y+gscale/2, (bar_width+3)*num_bars,CYAN); // Draw centre zero line if needed
  tft.drawFastVLine(x, y, gscale+1,CYAN);
...
void display_progress (String title, int percent) {
  ...
  int x_pos = 5; int y_pos = 65;
  int bar_width = 120; int bar_height = 10;
  tft.setTextScale(1);
  tft.fillRect(x_pos-5,y_pos-10,128,8,BLACK); // Clear titles
  display_text(title_pos,y_pos-10,title,GREEN,1,60);
  tft.drawRoundRect(x_pos,y_pos,bar_width+2,bar_height,3,GREEN); // Draw progress bar outline
  tft.fillRoundRect(x_pos+2,y_pos+1,percent*bar_width/100-2,bar_height-3,2,BLUE); // Draw progress
  delay(1000);
}
